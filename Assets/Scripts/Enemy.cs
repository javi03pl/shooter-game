...
public class Enemy : LivingEntity {
	...
	IEnumerator Attack() {
		...
		while (percent <= 1) {
			...
			if (percent >= .5f && !hasAppliedDamage) {
				...
				//StartCoroutine(AnimateDamageCamera());
			}
	...
	IEnumerator AnimateDamageCamera()
	{
		Color startColor = cam.backgroundColor;

		cam.backgroundColor = Color.Lerp(startColor , damageColor , 2f);
		yield return new WaitForSeconds(.7f);
		cam.backgroundColor = Color.Lerp(damageColor , startColor , 2f);
	}
